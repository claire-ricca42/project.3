...
int main ()
{
    ...
    // Start h;
    // h.setPlayername("Bob");
    // cout<<h.getPlayerName()<<endl;
    // h.setPartyNames("ryan","bill","Kyle", "claire");
    // cout<<h.getPartyMemberName("Kyle")<<endl;

    // cout<<h.getPartyMemberFullness(2)<<endl;
    // //say a misforturne happnens
    // h.setPartyFullness(50,50,39,50);
    // //kyle is the 3rd member so his fullness is effected, params will be done inside driver, not by player
    // cout<<h.getPartyMemberFullness(2)<<endl;
    ...
    for (int i = 0; i < 4; i++)
    {
        m.addRoom((rand()%10), (rand()%10));
        m.addNPC((rand()%10), (rand()%10));
    }
    m.displayMap();
    char direction;
    cout<<"which way"<<endl;
    ...
    m.move(direction);
    m.displayMap();
    while (!m.isDungeonExit(m.getPlayerRow(), m.getPlayerCol()))
    {
        if (!(m.isNPCLocation(m.getPlayerRow(), m.getPlayerCol())) && !(m.isNPCLocation(m.getPlayerRow(), m.getPlayerCol())))
        {
            /* code */
            //use this in main driver, call NormalSpace()
            cout<<"normal"<<endl;
            // ActionMenu a;
            // a.NormalSpace(m.getPlayerRow(), m.getPlayerCol(), erc, h, m);
        }
